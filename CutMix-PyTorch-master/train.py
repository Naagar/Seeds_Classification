...
parser.add_argument('--dataset', dest='dataset', default='seeds', type=str,
                    help='dataset (options: seeds, cifar10, cifar100, and imagenet)')
...
def main():
    ...
    if args.dataset.startswith('seed'):
        ...
        if args.dataset == 'seeds':
            train_loader= torch.utils.data.DataLoader(
                datasets.ImageFolder('data/train', transform=transform_train),
                batch_size=args.batch_size,shuffle=True, num_workers=args.workers, pin_memory=True)
            val_loader = torch.utils.data.DataLoader(
                datasets.ImageFolder('data/validation', transform=transform_test),
                batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
            numberofclass = 4
...
def adjust_learning_rate(optimizer, epoch):
    ...
    if args.dataset.startswith('seeds'):
