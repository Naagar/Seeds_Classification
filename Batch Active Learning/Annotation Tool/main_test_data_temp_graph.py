...
def card_body(card_id):
    ...
    # dbc.CardImg(src=paths_of_images[card_id] ,  top=True, style={"height": "125px", "width": "188px"}),
        # html.Img(src=paths_of_images[card_id] ,  style={"height": "125px", "width": "188px"}),
    # 'source':'data:image/png:base64,{}'.format(encoded_image)

    encoded_image = base64.b64encode(open(paths_of_images[card_id], mode='rb').read() )

    return [
        dcc.Graph(id = f'graph_img_{card_id}', 
                figure={'images': [{'source':encoded_image}] },
                   ),
...
# @app.callback(
#     dd.Output(component_id=f'img_0', component_property='style'),
#     [dd.Input(component_id=f'img_0', component_property='id')]
#     prevent_initial_call = True
# )
# def on_hover(id):
#     '''Only on clicking i.e n_clicks>=1 export button would work not from starting when the app runs'''
